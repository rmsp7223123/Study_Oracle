...
-- ROWNUM 또는 ROW_NUMBER() : 테이블에 존재하지 않는 의사컬럼(Pseudo Column)으로, select절과 where절에 사용
...
select *
from( select employee_id, last_name, salary from employees order by 3 desc)
where rownum <= 10;

select employee_id, last_name, salary,
rownum as, rank() over(order by salary desc) as salary_rank, dense_rank() over(order by salary desc) as salary_rank from employees;
